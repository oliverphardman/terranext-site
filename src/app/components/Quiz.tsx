...
function computeResult(q1: Q1, q2: Q2 | null, q3: Q3 | null): QuizResult {
	...
	if (q3 === "only-nextjs") {
		...
		return {
			...
			subtitle: "For a standalone Next.js app, managed platforms handle the heavy lifting for you.",
...
export function Quiz() {
	...
	if (step === 4 && q1) {
		...
		const isPerfect = result.recommendations.length === 1 && result.recommendations[0].perfect;

		return (
			<div className="max-w-2xl mx-auto text-center">
				<h3 className={`text-2xl font-bold ${isPerfect ? "text-accent" : ""}`}>{result.message}</h3>
				...
				{!isPerfect && (
					<ol className="mt-8 space-y-3 text-left max-w-md mx-auto">
						{result.recommendations.map((rec, i) => (
							<li key={rec.name} className={`flex gap-3 rounded-lg border p-3 border-border bg-card`}>
								<span
	...
	return (
		<div className="max-w-2xl mx-auto">
			{step === 1 && (
				<div>
					<h3 className="text-xl font-semibold text-center mb-6">Do you want to host on AWS?</h3>
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
						<OptionButton onClick={() => handleQ1("yes")}>Yes</OptionButton>
						<OptionButton onClick={() => handleQ1("doesnt-mind")}>I don&apos;t mind</OptionButton>
			...
			{step === 2 && (
				<div>
					<h3 className="text-xl font-semibold text-center mb-6">What type of project is this?</h3>
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
						<OptionButton onClick={() => handleQ2("individual")}>Individual or hobby</OptionButton>
						<OptionButton onClick={() => handleQ2("startup")}>For my startup</OptionButton>
						<OptionButton onClick={() => handleQ2("enterprise")}>Large organisation or enterprise</OptionButton>
					</div>
				</div>
			)}
			...
			{step === 3 && (
				<div>
					<h3 className="text-xl font-semibold text-center mb-6">What are your infrastructure plans?</h3>
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
						<OptionButton onClick={() => handleQ3("terraform")}>
							To manage infrastructure with Terraform or OpenTofu
						</OptionButton>
						<OptionButton onClick={() => handleQ3("only-nextjs")}>To host a Next.js app only</OptionButton>
